...
from app.core.dependencies import get_current_token, get_current_user, get_db
...
def register(payload: UserCreate, db: Session = Depends(get_db)) -> Any:
    """
...
@router.post("/login")
def login(payload: UserCreate, db: Session = Depends(get_db)) -> Any:
    """
    ...
    if not db_user:
        raise HTTPException(
            ...
            detail="이메일 또는 비밀번호가 잘못 입력 되었습니다."
        )
    ...
    if bcrypt.checkpw(
        ...
        db_token = crud_user.create_token(
            db=db,
            token=new_token,
...
def logout(
    ...
    :return:
    """
    if crud_user.delete_token(db=db,
                              token=current_token,
                              user_id=current_user.id):
        return {"message": "OK"}
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="로그인이 필요한 기능입니다."
        )
...
def refresh(
    ...
    try:
        ...
        db_token = crud_user.get_token_by_refresh(
            db=db, refresh_token=refresh_token, user_id=current_user.id
        )
        if validate_token(refresh_token) and db_token:
            new_token = create_token(email=current_user.email,
                                     is_refresh=False)
            refresh_token = create_token(email=current_user.email,
                                         is_refresh=True)
            new_db_token = crud_user.create_token(
                db=db,
                token=new_token,
                refresh_token=refresh_token,
                user_id=current_user.id,
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="'refresh_token' 키 값이 존재 하지 않습니다. ",
        )
